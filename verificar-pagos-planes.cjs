...
assert.match(firebase, /consultaSecundaria\(\(\) => getDoc\(doc\(db, COLECCIONES\.planesProfesionales, user\.uid\)\)/, "un error secundario del catálogo no debe ocultar un plan pagado");
assert.match(firebase, /planRegistro: planesPorUid\.get/, "el plan recuperado debe reflejarse en el perfil visible");
...
assert.match(aplicacion, /seleccionarPerfilProfesional\(data\.profesionales, data\.rol, data\.usuarioUid\)/, "el panel debe seleccionar el perfil por UID autenticado");
...
assert.match(firebase, /normalizarPerfilPropio\(\{ id: perfil\.id, data: perfil\.data\(\) \}, user\.uid\)/, "el perfil propio debe normalizarse con la cuenta autenticada");
