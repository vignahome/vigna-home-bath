(function () {
  ...
  function estadoClase(estado) {
    return estado === "Aprobado" || estado === "Verificado" || estado === "Firmado" ? "approved" : estado === "Rechazado" ? "rejected" : "pending";
  }
  ...
  function renderAdmin() {
    ...
    document.getElementById("admin-clientes").innerHTML = tableHtml(["ID", "Cliente", "Documento", "Ubicación", "Archivos", "Estado"], data.clientes.map((c) => [c.id, nombreCompleto(c), `${c.tipoDocumento} ${c.documento}`, `${c.departamento} - ${c.provincia} - ${c.distrito} · ${c.zona || "Sin zona"}`, `${c.documentosDeclarados || 0} declarados`, c.estado]));
    ...
    document.getElementById("admin-contratos").innerHTML = tableHtml(["ID", "Solicitud", "Profesional", "Total", "Estado", "Archivo"], data.contratos.map((c) => { const p = data.profesionales.find((x) => x.id === c.profesionalId); return [c.id, c.solicitudId, nombreCompleto(p), dinero(c.total), c.estado, c.archivoFirmado || "Pendiente"]; }));
  ...
  function createContract(quoteId, optionIndex) {
    ...
    const contract = { id: uid("CT"), solicitudId: quote.solicitudId, cotizacionId: quote.id, profesionalId: quote.profesionalId, clienteId: request.clienteId, opcion: option.nombre, total: option.precio, detalle: option.detalle, condiciones: quote.condiciones, version: 1, estado: "Pendiente de firma", creadoEn: nowIso(), archivoFirmado: "" };
  ...
  function openContract(id) {
    ...
    document.getElementById("contractDialogContent").innerHTML = `<div class="contract-sheet"><p class="eyebrow">CONTRATO ${escapar(contract.id)} · VERSIÓN ${contract.version}</p><h1>Contrato de prestación de servicios</h1><p><b>Cliente:</b> ${escapar(nombreCompleto(client))} · ${escapar(client?.tipoDocumento)} ${escapar(client?.documento)}</p><p><b>Profesional:</b> ${escapar(nombreCompleto(professional))} · ${escapar(professional?.tipoDocumento)} ${escapar(professional?.documento)}</p><h2>Objeto y alcance</h2><p>${escapar(request?.descripcion || "Servicio acordado")}</p><p><b>Opción:</b> ${escapar(contract.opcion)} · <b>Total:</b> ${dinero(contract.total)}</p><p>${escapar(contract.detalle)}</p><h2>Lugar y plazo</h2><p>${escapar(request?.departamento)}, ${escapar(request?.provincia)}, ${escapar(request?.distrito)} · Fecha preferida: ${escapar(request?.fecha)}</p><h2>Condiciones, pagos, garantía y exclusiones</h2><p>${escapar(contract.condiciones)}</p><h2>Estado</h2><p>${escapar(contract.estado)}${contract.archivoFirmado ? ` · Archivo: ${escapar(contract.archivoFirmado)}` : ""}</p><div class="signed-upload no-print"><button class="secondary-button" type="button" onclick="window.print()">Imprimir contrato</button><label>Subir contrato firmado<input id="signedContractFile" type="file" accept=".pdf,image/*"></label><button class="gold-button" data-upload-contract="${contract.id}">Registrar documento firmado</button></div></div>`;
  ...
  document.getElementById("formCotizacion").addEventListener("submit", (event) => {
    ...
    ], condiciones: form.get("condiciones"), version: 1, estado: "Enviada", creadoEn: nowIso() };
