...
(function () {
  ...
  function renderPanel() {
    ...
    if (notaPlan) notaPlan.textContent = vigente
      ? "Tu plan está activo. No se realizará ningún cobro adicional mientras siga vigente."
      : (window.Capacitor?.isNativePlatform?.()
        ? "El pago se procesará mediante la tienda de tu dispositivo."
        : "Serás dirigido al entorno seguro de Mercado Pago. VIGNA no almacena los datos de tu tarjeta.");
    const registradas = new Set(especialidades.map((item) => String(item.profesion || "").toLocaleLowerCase("es")));
    const disponibles = [...new Set([...(p.profesiones || []), ...profesiones])].filter((item) => !registradas.has(String(item).toLocaleLowerCase("es")));
    const formularioAlta = disponibles.length ? `<form class="specialty-editor specialty-create" data-specialty-create><div class="execution-card-head"><div><p class="eyebrow">NUEVA PROFESIÓN</p><h3>Registrar para revisión</h3></div><span class="status-chip pending">Pendiente</span></div><p class="private-note">Los perfiles anteriores deben registrar aquí cada profesión. Administración revisará la información y los certificados antes de publicarla.</p><label>Profesión<select name="profesion" required><option value="">Selecciona una profesión</option>${disponibles.map((item) => `<option${item === p.profesionPrincipal ? " selected" : ""}>${escapar(item)}</option>`).join("")}</select></label><label><input name="principal" type="checkbox" ${especialidades.length ? "" : "checked"}> Profesión principal</label><label>Años de experiencia<input name="experiencia" type="number" min="0" max="80" value="${Number(p.experiencia || 0)}"></label><label>Experiencia específica<textarea name="descripcion" maxlength="1500" rows="3" placeholder="Describe tu experiencia en esta profesión."></textarea></label><button class="gold-button" type="submit">Registrar profesión</button></form>` : '<div class="empty-state specialty-complete"><p>Todas las profesiones del perfil ya están registradas.</p></div>';
    const editores = especialidades.map((item) => `<form class="specialty-editor" data-specialty-form="${escapar(item.id)}"><div class="execution-card-head"><h3>${escapar(item.profesion)}</h3><span class="status-chip ${estadoClase(item.estado)}">${escapar(item.estado)}</span></div><label><input name="principal" type="checkbox" ${item.principal ? "checked" : ""}> Profesión principal</label><label>Años de experiencia<input name="experiencia" type="number" min="0" max="80" value="${Number(item.experiencia || 0)}"></label><label>Experiencia específica<textarea name="descripcion" maxlength="1500" rows="3">${escapar(item.descripcion || "")}</textarea></label><label>Certificados o constancias (máximo 6)<input name="evidencias" type="file" accept="image/*,application/pdf" multiple></label><div class="specialty-evidence-list">${(item.evidencias || []).map((evidencia) => `<button class="execution-file" type="button" data-open-specialty-file="${escapar(item.id)}" data-file-path="${escapar(evidencia.ruta)}">${escapar(evidencia.nombre)}</button>`).join("")}</div>${item.observacion ? `<p class="private-note">Observación: ${escapar(item.observacion)}</p>` : ""}<p class="private-note">Si cambias la experiencia o adjuntas constancias, la profesión volverá a revisión.</p><button class="gold-button" type="submit">Guardar especialidad</button></form>`).join("");
    document.getElementById("professionalSpecialties").innerHTML = formularioAlta + editores;
    renderQuotes();
  }
  ...
  function renderAdmin() {
    ...
    document.getElementById("admin-profesionales").innerHTML = tableHtml(["ID", "Profesional", "Profesión principal", "Cobertura", "Documentos", "Estado", "Acciones"], data.profesionales.map((p) => {
      const especialidades = p.especialidades || [];
      const registradas = new Set(especialidades.map((item) => String(item.profesion || "").toLocaleLowerCase("es")));
      const profesionesFaltantes = (p.profesiones || []).filter((item) => !registradas.has(String(item).toLocaleLowerCase("es")));
      const migracion = profesionesFaltantes.length ? `<span class="private-note">Sin registro independiente: ${profesionesFaltantes.map(escapar).join(", ")} <button class="tiny-button" data-admin-migrate-specialties="${escapar(p.id)}">Registrar para revisión</button></span>` : "";
      const plan = p.planRegistro || {};
      const vencimientoPlan = Date.parse(plan.venceEn || p.planVenceEn || "");
      const planActivo = plan.estado === "Activo" && (!Number.isFinite(vencimientoPlan) || vencimientoPlan > Date.now());
      const accionPlan = planActivo
        ? `<span class="status-chip approved">Plan ${escapar(plan.tipo || p.plan || "profesional")} activo${Number.isFinite(vencimientoPlan) ? ` hasta ${escapar(new Date(vencimientoPlan).toLocaleDateString("es-PE"))}` : ""}</span>`
        : `<button class="tiny-button" data-admin-activate-plan="${p.id}" data-plan-type="${escapar(plan.tipo || "Mensual")}">${plan.estado === "Activo" ? "Renovar plan" : "Activar plan"}</button>`;
      return [
  ...
  function nuevaFilaMaterialCotizacion() {
    const fila = document.createElement("div");
    fila.className = "quote-material-row";
    fila.dataset.quoteMaterialRow = "";
    fila.innerHTML = '<label>Material<input name="materialNombre" placeholder="Ej. Manguera flexible" required></label><label>Cantidad<input name="materialCantidad" type="number" min="0.01" step="0.01" value="1" required></label><label>Unidad<select name="materialUnidad" required><option>Unidad</option><option>Metro</option><option>Bolsa</option><option>Caja</option><option>Tubo</option><option>Juego</option><option>Otro</option></select></label><label>Precio unitario (S/)<input name="materialPrecioUnitario" type="number" min="0" step="0.01" placeholder="Ej. 20.00" required></label><label>Subtotal (S/)<input name="materialSubtotal" type="number" readonly placeholder="0.00"></label><button class="tiny-button quote-material-remove" type="button" data-remove-quote-material aria-label="Eliminar material">Eliminar</button>';
    return fila;
  }

  function calcularCotizacion() {
    let materiales = 0;
    document.querySelectorAll("[data-quote-material-row]").forEach((fila) => {
      const cantidad = Number(fila.querySelector('[name="materialCantidad"]')?.value || 0);
      const precioUnitario = Number(fila.querySelector('[name="materialPrecioUnitario"]')?.value || 0);
      const subtotal = Number((cantidad * precioUnitario).toFixed(2));
      const campoSubtotal = fila.querySelector('[name="materialSubtotal"]');
      if (campoSubtotal) campoSubtotal.value = subtotal ? subtotal.toFixed(2) : "";
      materiales += subtotal;
    });
    materiales = Number(materiales.toFixed(2));
    const campoMateriales = document.querySelector('[name="cotizacionMateriales"]');
    if (campoMateriales) campoMateriales.value = materiales ? materiales.toFixed(2) : "";
  ...
  function detalleMaterialesFormulario(form) {
    ...
    const precios = form.getAll("materialPrecioUnitario");
    return nombres.map((nombre, index) => {
      const cantidad = Number(cantidades[index] || 0);
      const precioUnitario = Number(precios[index] || 0);
      return { nombre: String(nombre || "").trim(), cantidad, unidad: String(unidades[index] || "Unidad"), precioUnitario, subtotal: Number((cantidad * precioUnitario).toFixed(2)) };
  ...
  function openContract(id) {
    ...
    let actions = `<button class="secondary-button" type="button" data-download-contract-pdf="${contract.id}">Descargar PDF contractual</button><button class="secondary-button" type="button" onclick="window.print()">Imprimir expediente actualizado</button><a class="secondary-button button-link" href="plantillas/plantilla-productos-paso-a-paso-vigna.xlsx" download>Descargar plantilla Excel</a>`;
    ...
    if (!contract.archivoFirmado) {
      actions += `<label>Subir documento contractual para confirmación<input id="signedContractFile" type="file" accept=".pdf,image/*"></label><button class="gold-button" data-upload-contract="${contract.id}">Registrar documento para confirmar</button>`;
    } else {
      const confirmaciones = contract.confirmacionesFirma || {};
      const tieneHuella = /^[a-f0-9]{64}$/i.test(String(contract.documentoHashSha256 || ""));
      const confirmacionCompleta = tieneHuella && confirmaciones.cliente === true && confirmaciones.profesional === true;
      const etiquetaDocumento = !tieneHuella ? "Abrir documento histórico" : confirmacionCompleta ? "Abrir documento confirmado" : "Abrir documento para confirmar";
      actions += `<button class="secondary-button" type="button" data-open-signed-contract="${contract.id}">${etiquetaDocumento}</button>`;
      if (!tieneHuella) {
        actions += `<div class="signature-verification"><h2>Documento histórico</h2><p>Este archivo fue registrado antes de incorporar la huella SHA-256 y la confirmación bilateral. No existen confirmaciones pendientes para este documento.</p><p>El archivo original permanece inmutable. El estado vigente y el cierre se acreditan en este expediente.</p></div>`;
      } else {
        actions += `<div class="signature-verification"><h2>Confirmación bilateral</h2><p>Cliente: <b>${confirmaciones.cliente ? "Confirmado" : "Pendiente"}</b> · Profesional: <b>${confirmaciones.profesional ? "Confirmado" : "Pendiente"}</b></p><p>El archivo original permanece inmutable y conserva el estado que tenía al cargarse. El estado vigente, las confirmaciones y el cierre se acreditan en este expediente.</p><p>Huella SHA-256: <code>${escapar(contract.documentoHashSha256)}</code></p>${contract.estado === "Pendiente de confirmación" && ((data.rol === "cliente" && !confirmaciones.cliente) || (data.rol === "profesional" && !confirmaciones.profesional)) ? `<label class="acceptance-check"><input id="contractHashAcceptance" type="checkbox" required> Abrí el archivo, verifiqué su contenido y confirmo exactamente esta versión inmutable.</label><button class="gold-button" type="button" data-confirm-signed-contract="${contract.id}">Confirmar documento</button>` : ""}</div>`;
      }
    }
    ...
    const ubicacion = contract.ubicacion || {};
    const desglose = contract.desglose || {};
    const fechaEmision = contract.creadoEn ? new Date(contract.creadoEn).toLocaleString("es-PE") : "No registrada";
    document.getElementById("contractDialogContent").innerHTML = `<div class="contract-sheet"><div class="contract-brand"><img src="images/logo/ChatGPT Image 26 may 2026, 11_05_03 p.m..png" alt="VIGNA Home & Bath"><strong>Profesionales Vigna’s</strong></div><div class="contract-identity"><p class="eyebrow">CONTRATO ${escapar(contract.id)} · VERSIÓN ${contract.version}</p><p><b>Emitido:</b> ${escapar(fechaEmision)} · <b>Solicitud:</b> ${escapar(contract.solicitudId)} · <b>Cotización:</b> ${escapar(contract.cotizacionId)} v${Number(contract.cotizacionVersion || 1)}</p></div><h1>Contrato de prestación de servicios</h1><p><b>Cliente:</b> ${escapar(contract.clienteNombre || nombreCompleto(client))} · ${escapar(contract.clienteTipoDocumento || client?.tipoDocumento)} ${documentValue(clientDocument)}</p><p><b>Profesional:</b> ${escapar(contract.profesionalNombre || nombreCompleto(professional))} · ${escapar(contract.profesionalTipoDocumento || professional?.tipoDocumento)} ${documentValue(professionalDocument)}</p><h2>Objeto, alcance y entregables</h2><p>${escapar(contract.descripcionSolicitud || request?.descripcion || "Servicio acordado")}</p><p>${escapar(contract.detalle)}</p><h2>Precio y pagos</h2><p><b>Opción:</b> ${escapar(contract.opcion)} · <b>Total:</b> ${dinero(contract.total)}</p><table class="contract-price-table"><tbody><tr><th>Materiales</th><td>${dinero(desglose.materiales)}</td></tr><tr><th>Mano de obra</th><td>${dinero(desglose.manoObra)}</td></tr><tr><th>Transporte, permisos u otros</th><td>${dinero(desglose.otros)}</td></tr><tr><th>Total</th><td><b>${dinero(contract.total)}</b></td></tr></tbody></table><p><b>Forma de pago:</b> ${escapar(contract.formaPago || "Según acuerdo documentado entre las partes.")}</p><p>Los pagos del servicio son declarados por las partes; VIGNA no custodia el dinero ni sustituye el comprobante tributario aplicable.</p><h2>Lugar, plazo y responsabilidades</h2><p>${escapar(ubicacion.departamento || request?.departamento)}, ${escapar(ubicacion.provincia || request?.provincia)}, ${escapar(ubicacion.distrito || request?.distrito)} · ${escapar(ubicacion.direccion || "Dirección privada registrada en el expediente")}${ubicacion.referencia ? ` · ${escapar(ubicacion.referencia)}` : ""}</p><p><b>Inicio preferido:</b> ${escapar(ubicacion.fecha || request?.fecha || "Por coordinar")}${ubicacion.fechaFin ? ` · <b>Fin esperado:</b> ${escapar(ubicacion.fechaFin)}` : ""}</p><p><b>Materiales:</b> ${escapar(contract.responsableMaterialesSolicitud || contract.responsableMateriales || "Por definir")} · <b>Restricciones y acceso:</b> ${escapar(contract.restricciones || "Sin restricciones adicionales declaradas.")}</p><h2>Garantía estructurada</h2>${garantiaHtml}<h2>Condiciones, exclusiones, cambios y cancelación</h2><p>${escapar(contract.condiciones)}</p><p><b>Exclusiones:</b> ${escapar(contract.exclusiones || "Sin exclusiones adicionales declaradas.")}</p><p>Todo cambio de alcance, precio o plazo requiere una orden escrita aceptada en el expediente. Las pausas, cancelaciones, incidencias y reclamos deben registrarse con motivo y evidencia. Ninguna modificación reemplaza silenciosamente esta versión.</p><h2>Privacidad y evidencias</h2><p>Las partes autorizan el tratamiento restringido de los datos y archivos necesarios para ejecutar, acreditar y resolver este servicio conforme a los términos y la política de privacidad de VIGNA. La publicación de fotografías requiere autorización independiente.</p><h2>Plan de productos y ejecución paso a paso (opcional)</h2>${anexoHtml}<div class="contract-signatures"><div><span></span><b>Firma del cliente</b><small>${escapar(contract.clienteNombre || nombreCompleto(client))} · ${escapar(contract.clienteTipoDocumento || client?.tipoDocumento)} ${documentValue(clientDocument)}</small></div><div><span></span><b>Firma del profesional</b><small>${escapar(contract.profesionalNombre || nombreCompleto(professional))} · ${escapar(contract.profesionalTipoDocumento || professional?.tipoDocumento)} ${documentValue(professionalDocument)}</small></div></div><div class="contract-status-print"><h2>Estado al imprimir</h2><p>${escapar(contract.estado)}${contract.archivoFirmado ? ` · Archivo: ${escapar(contract.archivoFirmado)}` : ""}</p></div>${timeline}${executionHtml}${comunicacionHtml}${evidenceHtml}${reviewHtml}<div class="signed-upload no-print">${actions}</div></div>`;
  ...
  document.addEventListener("click", (event) => {
    ...
    const addMaterial = event.target.closest("[data-add-quote-material]"); if (addMaterial) { document.getElementById("quoteMaterialsList")?.appendChild(nuevaFilaMaterialCotizacion()); calcularCotizacion(); }
    const removeMaterial = event.target.closest("[data-remove-quote-material]"); if (removeMaterial) { const filas = document.querySelectorAll("[data-quote-material-row]"); if (filas.length === 1) return toast("La cotización debe conservar al menos un material."); removeMaterial.closest("[data-quote-material-row]")?.remove(); calcularCotizacion(); }
  ...
  document.getElementById("formCotizacion").addEventListener("input", (event) => {
    if (!/^(materialCantidad|materialPrecioUnitario|cotizacionManoObra|cotizacionOtros)$/.test(event.target.name || "")) return;
    calcularCotizacion();
  });
  ...
  document.getElementById("formProfesional").addEventListener("submit", (event) => {
    ...
    if (selected.length > 10) return toast("Puedes registrar como máximo 10 profesiones.");
  ...
  document.getElementById("formCotizacion").addEventListener("submit", (event) => {
    ...
    const anteriores = data.cotizaciones.filter((item) => item.solicitudId === form.get("solicitudId") && item.profesionalId === panelProfesionalId).sort((a, b) => Number(b.version || 0) - Number(a.version || 0)); const anterior = anteriores[0]; const nuevaId = uid("CO"); const opcion = (prefijo, nombre) => { const materialesDetalle = detalleMaterialesFormulario(form); const materiales = Number(materialesDetalle.reduce((total, item) => total + item.subtotal, 0).toFixed(2)); const manoObra = Number(form.get(`${prefijo}ManoObra`) || 0); const otros = Number(form.get(`${prefijo}Otros`) || 0); return { nombre, precio: Number((materiales + manoObra + otros).toFixed(2)), detalle: form.get(`${prefijo}Detalle`), materiales, materialesDetalle, manoObra, otros, duracion: form.get(`${prefijo}Duracion`) }; };
    ...
    guardar("Cotización enviada", `${q.id} para ${q.solicitudId}`, nombreCompleto(data.profesionales.find((p) => p.id === panelProfesionalId))); event.currentTarget.reset(); document.getElementById("quoteMaterialsList").innerHTML = ""; document.getElementById("quoteMaterialsList").appendChild(nuevaFilaMaterialCotizacion()); calcularCotizacion(); toast("Cotización enviada. El cliente podrá descargarla en Excel.");
  });
